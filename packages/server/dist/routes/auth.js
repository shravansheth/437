...
router.post("/login", (req, res) => {
  ...
  if (!username || !password) {
    ...
    import_credential_svc.default.verify(username, password).then((username2) => generateAccessToken(username2)).then((token) => res.status(200).send({ token })).catch(() => res.status(401).send("Unauthorized"));
  }
});
...
router.post("/register", (req, res) => {
  ...
  if (typeof username !== "string" || typeof password !== "string") {
    ...
    import_credential_svc.default.create(username, password).then((creds) => generateAccessToken(creds.username)).then((token) => res.status(201).send({ token })).catch((err) => res.status(409).send({ error: err }));
  }
});
router.post("/login", (req, res) => {
  const { username, password } = req.body;
